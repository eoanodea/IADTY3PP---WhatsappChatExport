<template>
    <div>
        <!-- Select Service -->
        <cv-dropdown label="Select a Service" value="service" name="service" id="service" v-model="serviceId" class="cv-dropdown" placeholder="Select a Service">
            <cv-dropdown-item v-for="service in services" v-bind:key="service.id" :value="service.id.toString()">
                <span style="color:#D1D2D8;">{{service.title}}</span>
            </cv-dropdown-item>
        </cv-dropdown>

        <!-- Select Client -->
        <br/>
        <cv-dropdown label="Select a Client" value="client" name="client" id="client" v-model="clientId" class="cv-dropdown" placeholder="Select a Client">
            <cv-dropdown-item v-for="client in users" v-bind:key="client.id" :value="client.id.toString()">
                <span style="color:#D1D2D8;">{{client.first_name}}</span> <span class="dropDownSpan logoCol">{{client.last_name}}</span>
            </cv-dropdown-item>
        </cv-dropdown>
